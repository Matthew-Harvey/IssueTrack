...
export default async function ValidateRegister(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (canAdd === true) {
      ...
      await addDoc(collection(firestore, "users"), {name: username, pass: hash, email: email, status: "", teams: "", teams_name: "", created: moment().format("DD-MM-YYYY hh:mm:ss")}).then(function(docRef) { res.status(200).json({isAdded: true, id: docRef.id, error: ""});})
    } else {
