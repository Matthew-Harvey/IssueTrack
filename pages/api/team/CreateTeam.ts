...
export default async function CreateTeam(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
      await addDoc(teams_collection, {name: teamname, id: teamid, members: teammembers, created: moment().format("DD-MM-YYYY hh:mm:ss"), creator: userid, overview: teamoverview}).then(function(docRef) { 
