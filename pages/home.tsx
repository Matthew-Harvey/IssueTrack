...
import { Box, Breadcrumbs, Button, Card, Grid, Link } from '@mui/material';
...
export const getServerSideProps = async (ctx) => {
    ...
    return { props: {auth: getAuth.data.isAuth, userid: username_cookie, this_userteams: getUserTeams.data}};
}

export default function home({auth, userid, this_userteams}) {

    const userteams = this_userteams.teams.toString().split(",");
    const userteamnames = this_userteams.teams_name.toString().split(",");
    const usercreated = this_userteams.created;
    const userstatus = this_userteams.status;
                                            ...
                                            <>
                                                <Grid item={true} key={_key} xs={6} sm={6} md={4} lg={3} style={{padding: "1em", justifyContent: "center", textAlign: "center"}}>
                                                    <Card variant="outlined" style={{background: getgradient, padding: "1em"}}>
                                                            <h3 style={{color: "white", padding: "1em"}}>{userteamnames[_key]}</h3>
                                                            <Link href={teamurl}>
